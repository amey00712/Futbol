...
Text headerText(var text) {
  return Text(
    text,
    maxLines: 2,
    style: TextStyle(fontFamily: fontBold, fontSize: 22, color: t2_textColorPrimary),
  );
}
...
Text subHeadingText(var text) {
  return Text(
    text,
    style: TextStyle(fontFamily: fontBold, fontSize: 17.5, color: t2_textColorSecondary),
  );
}

Widget text(var text, {var fontSize = textSizeLargeMedium, textColor = t2_textColorSecondary, var fontFamily = fontRegular, var isCentered = false, var maxLine = 1, var latterSpacing = 0.5, var fontWeight = FontWeight.normal}) {
  return Text(text,
...
Widget ring(String description) {
  return Column(
    ...
    children: <Widget>[
      ...
      SizedBox(height: 16),
      text(description, textColor: t2TextColorPrimary, fontSize: textSizeNormal, fontFamily: fontSemibold, isCentered: true, maxLine: 2)
    ],
  );
}
...
BoxDecoration boxDecoration({double radius = 2, Color color = Colors.transparent, Color bgColor = t2_white, var showShadow = false}) {
...
Container T9EditTextStyle(var hintText, TextEditingController controller, TextInputType type ,bool isEnabled,{isPassword = false}) {
  return Container(
    decoration: boxDecoration(radius: 40, showShadow: true, bgColor: t2_white),
    child: TextFormField(
      readOnly: isEnabled,
